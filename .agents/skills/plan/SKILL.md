---
name: plan
description: Use this skill when the user wants to break down an epic into tasks, says "plan this epic", "create issues", "destrinchar o épico", or provides an epic number to plan. Reads the PRD.md and codebase, generates a SPEC.md, and creates atomic GitHub issues ready for implementation.
---
...
2. Read `PRD.md` to understand the full product context behind this epic
3. Read `AGENTS.md` to understand project conventions, stack, and rules
4. Read `docs/business-rules.md` to identify which rule modules are relevant to this epic
5. Load and read only the relevant module files listed in `docs/business-rules.md`
6. If the epic introduces rules not covered by any existing module:
   - Create a new module file in `/docs` following the existing pattern
   - Register it in `docs/business-rules.md`
7. Analyze the existing codebase structure to understand patterns and relevant files
8. Generate a `SPEC.md` file in the repository root with:
...
9. For each task that will be created, identify if it involves UI:
   - If yes, ask the human: "A task '[task title]' envolve UI. Você já tem o link do Figma para essa tela?"
   - If the human provides a link, include it in the issue body as `Design: [link]`
   - If the human does not have it yet, create the issue with `Design: pending` and label `needs-design`
   - If the task does not involve UI, skip this step
10. Create atomic GitHub issues linked to the epic, each containing:
    - Clear title
    - Affected files
    - Exactly what to do
    - Definition of done
